...
var TestPageUI=(function(_super){
	...
	TestPageUI.uiView={"type":"View","props":{"width":600,"height":400},"child":[{"type":"Tab","props":{"y":132,"x":72,"selectedIndex":2},"child":[{"type":"Button","props":{"y":183,"x":184,"width":119,"var":"buttonTest","skin":"template/ButtonTab/btn_LargeTabButton_Left.png","name":"item0","labelSize":36,"labelColors":"#007AFF,#007AFF,#FFFFFF","label":"Button","height":53}}]}]};
	return TestPageUI;
})(View)
...
var TestView=(function(_super){
	function TestView(){
		...
		this.px=NaN;
		this.py=NaN;
		this.diffOffsetX=NaN;
		this.diffOffsetY=NaN;
		this.t1=0;
		this.targetPivotX=0;
		this.targetPivotY=0;
		this.t10=false;
		TestView.__super.call(this);
		Laya.CONCHVER && (conch.disableMultiTouch=false);
		this.tiledMap=new TiledMap();
		this.tiledMap.createMap("res/tiledmap/f.json",new Rectangle(0,0,1600,1600),Handler.create(this,this.mapLoaded),null);
		Laya.stage.on("mousedown",this,this.mouseDown);
		Laya.stage.on("mouseup",this,this.mouseUp);
		Laya.stage.on("mousemove",this,this.mouseMove);
		this.spriteBox=new Sprite();
		this.spriteBox.width=30;
		this.spriteBox.height=30;
		Laya.stage.addChild(this.spriteBox);
		this.spriteBox.zOrder=1000;
		Laya.stage.on("click",this,this.onStageClick);
	}

	__class(TestView,'view.TestView',_super);
	var __proto=TestView.prototype;
	__proto.scaleTest=function(){
		this.t10=false;
		this.tiledMap.mapSprite().scale(this.tiledMap.mapSprite().scaleX+0.1,this.tiledMap.mapSprite().scaleX+0.1,false);
	}

	__proto.test3=function(){
		var Range=this.MP().width-0;
		var Rand=Math.random();
		var randomPX=0+Math.round(Rand *Range);
		var randomPY=0+Math.round(Rand*Range);
		this.t10=false;
		randomPX=this.targetPivotX;
		randomPY=this.targetPivotY;
		this.MP().pivot(this.targetPivotX,this.targetPivotY);
		console.log("random px:"+randomPX+",random py:"+randomPY);
		var newX=0;
		var newY=0;
		if(randomPX>=this.prePivotX){
			newX=this.MP().x+Math.abs(randomPX-this.prePivotX)*this.MP().scaleX;
			}else{
			newX=this.MP().x-Math.abs(randomPX-this.prePivotX)*this.MP().scaleX;
		}
		if(randomPY>=this.prePivotY){
			newY=this.MP().y+Math.abs(randomPY-this.prePivotY)*this.MP().scaleX;
			}else{
			newY=this.MP().y-Math.abs(randomPY-this.prePivotY)*this.MP().scaleX;
		}
		this.MP().pos(newX,newY);
		this.prePivotX=randomPX;
		this.prePivotY=randomPY;
		this.spriteBox.graphics.clear();
		this.spriteBox.graphics.drawRect(this.MP().x,this.MP().y,this.spriteBox.width,this.spriteBox.height,"#FF5500",5);
	}

	__proto.test2=function(){{
			switch(this.tb){
				case 0:
					this.tiledMap.mapSprite().pivot(144,144);
					this.tiledMap.mapSprite().pos(304,304);
					this.tb++;
					break ;
				case 1:
					console.log(this.tiledMap.mapSprite().x+","+this.tiledMap.mapSprite().y);
					this.tiledMap.mapSprite().pivot(88,88);
					this.tiledMap.mapSprite().pos(304-(144-88)*this.MP().scaleX,304-(144-88)*this.MP().scaleY);
					break ;
				}
		}
		this.txt1.text=this.tiledMap.mapSprite().width+","+this.tiledMap.mapSprite().height;
		console.log("xx:"+this.tiledMap.mapSprite().x+","+this.tiledMap.mapSprite().y);
	}

	__proto.scaleTest2=function(){
		this.tiledMap.mapSprite().rotation+=2;
	}
	...
	*获取中心点
	*/
	__proto.getCenterPoint=function(x,y){
	...
	__proto.mouseDown=function(e){
		...
		if(false){
			if(!this.t10){
				this.t10=true;
				if(Laya.stage.mouseX>=this.MP().x){
					this.targetPivotX=this.prePivotX+Math.abs(Laya.stage.mouseX-this.MP().x)/this.MP().scaleX;
					if(this.targetPivotX>this.MP().width){
						console.log("pivot x 超出限定区域");
					}
					}else{
					this.targetPivotX=this.prePivotX-Math.abs(Laya.stage.mouseX-this.MP().x)/this.MP().scaleX;
					if(this.targetPivotX<0){
						console.log("pivot x 小于限定区域");
					}
				}
				if(Laya.stage.mouseY>=this.MP().y){
					this.targetPivotY=this.prePivotY+Math.abs(Laya.stage.mouseY-this.MP().y)/this.MP().scaleY;
					if(this.targetPivotY>this.MP().height){
						console.log("pivot y 超出限定区域");
					}
					}else{
					this.targetPivotY=this.prePivotY-Math.abs(Laya.stage.mouseY-this.MP().y)/this.MP().scaleY;
					if(this.targetPivotY<0){
						console.log("pivot y 小于限定区域");
					}
				}
				console.log("targetPX:"+this.targetPivotX+",targetPY:"+this.targetPivotY);
			}
		};
		var touches=e.touches;
		if(touches && touches.length==1){
			this.txt.text="down"+this.t1;
			this.pressDown=true;
			this.px=touches[0].stageX;
			this.py=touches[0].stageY;
			this.diffOffsetX=this.px-this.tiledMap.mapSprite().x;
			this.diffOffsetY=this.py-this.tiledMap.mapSprite().y;
			this.tiledMap.mapSprite().pos(this.px-this.diffOffsetX,this.py-this.diffOffsetY);
		}
		if(touches && touches.length==2){
			this.pressDown=false;
			this.lastDistance=this.getDistance(touches);
			var touchCenterX=this.getCenterPoint(touches[0].stageX,touches[1].stageX);
			var touchCenterY=this.getCenterPoint(touches[0].stageY,touches[1].stageY);
			var tPivotX=this.getPivotXInMap(touchCenterX);
			var tPivotY=this.getPivotYInMap(touchCenterY);
			this.targetPivotX=tPivotX;
			this.targetPivotY=tPivotY;
			this.test3();
		}
	}
	...
	*获取点击位置的中心点X
	*/
	__proto.getPivotXInMap=function(val){
		if(val>=this.MP().x){
			return this.prePivotX+Math.abs(val-this.MP().x)/this.MP().scaleX;
			}else{
			return this.prePivotX-Math.abs(val-this.MP().x)/this.MP().scaleX;
		}
	}
	...
	*获取点击位置的中心点Y
	*/
	__proto.getPivotYInMap=function(val){
		if(val>=this.MP().y){
			return this.prePivotY+Math.abs(val-this.MP().y)/this.MP().scaleY;
			}else{
			return this.prePivotY-Math.abs(val-this.MP().y)/this.MP().scaleY;
		}
	}
	...
	// }
	__proto.checkPivotX=function(val){
		if(val>this.MP().width){
			console.log("pivot x 超出限定区域");
			return false;
		}
		if(val<0){
	...
	__proto.checkPivotY=function(val){
		if(val>this.MP().height){
			console.log("pivot y 超出限定区域");
			return false;
		}
		if(val<0){
	...
	__proto.mouseMove=function(e){
		...
		if(touches && touches[0]){
			if(this.pressDown){
				this.t1++;
				this.px=Laya.stage.mouseX;
				this.py=Laya.stage.mouseY;
				this.tiledMap.mapSprite().pos(this.px-this.diffOffsetX,this.py-this.diffOffsetY);
				if(this.tiledMap.mapSprite().x>=this.TargetPovitXByScale()){
					this.tiledMap.mapSprite().x=this.TargetPovitXByScale();
				}
				if(this.tiledMap.mapSprite().y>=this.TargetPovitYByScale()){
					this.tiledMap.mapSprite().y=this.TargetPovitYByScale();
				};
				var diffWidthHorizontal=Laya.stage.width-this.MapWidth();
				if(diffWidthHorizontal>=0){
					diffWidthHorizontal=0;
				};
				var diffMW_SW=this.SW()-this.MapWidth();
				if(this.MP().x<=diffMW_SW+this.TargetPovitXByScale()){
					this.MP().x=diffMW_SW+this.TargetPovitXByScale();
				};
				var diffMH_SH=this.SH()-this.MapHeight();
				if(this.MP().y<=diffMH_SH+this.TargetPovitYByScale()){
					this.MP().y=diffMH_SH+this.TargetPovitYByScale();
				}
				this.txt.text=this.tiledMap.mapSprite().x+","+this.tiledMap.mapSprite().y;
			}
		}
	}
	...
	*/
	__proto.TargetPovitXByScale=function(){
		return this.targetPivotX*this.MP().scaleX;
	}

	__proto.TargetPovitYByScale=function(){
		return this.targetPivotY*this.MP().scaleY;
	}

	__proto.MX=function(){
		return this.tiledMap.mapSprite().x;
	}

	__proto.MY=function(){
		return this.tiledMap.mapSprite().y;
	}
	...
	__proto.UpdateScale=function(x){
		this.tiledMap.mapSprite().scale(this.tiledMap.mapSprite().scaleX+x,this.tiledMap.mapSprite().scaleX+x);
		if(this.tiledMap.mapSprite().scaleX<=1){
			this.tiledMap.mapSprite().scaleX=1;
			this.tiledMap.mapSprite().scale(1,1);
		}
		else
		if(this.tiledMap.mapSprite().scaleX>=2){
			this.tiledMap.mapSprite().scaleX=2;
			this.tiledMap.mapSprite().scale(2,2);
		}
		if(x<0){
			if(this.tiledMap.mapSprite().x>=this.TargetPovitXByScale()){
				this.tiledMap.mapSprite().x=this.TargetPovitXByScale();
			...
			var diffSW_MW=this.SW()-this.MapWidth();
			if(this.MX()<=diffSW_MW+this.TargetPovitXByScale()){
				this.MP().x=diffSW_MW+this.TargetPovitXByScale();
			};
			var diffSH_MH=this.SH()-this.MapHeight();
			if(this.MY()<=diffSH_MH+this.TargetPovitYByScale()){
				this.MP().y=diffSH_MH+this.TargetPovitYByScale();
			}
		}
	}

	// }
	__proto.MapWidth=function(){
		return this.tiledMap.width*this.tiledMap.mapSprite().scaleX;
	}

	__proto.MapHeight=function(){
		return this.tiledMap.height*this.tiledMap.mapSprite().scaleX;
	}

	__proto.TiledWidth=function(){
		return this.tiledMap.tileWidth*this.tiledMap.mapSprite().scaleX;
	}
	...
	// tf=!tf;
	__proto.MP=function(){
		return this.tiledMap.mapSprite();
	}

	__proto.mapLoaded=function(e){
		this.layer=this.tiledMap.getLayerByIndex(0);
		var radiusX=32;
		var radiusY=Math.tan(180 / Math.PI *30)*radiusX;
		var color="#FF7F50";
		this.tiledMap.mapSprite().size(1600,1600);
		this.tiledMap.mapSprite().pos(0,0);
		this.tiledMap.mapSprite().pivot(0,0);
		this.targetPivotX=0;
		this.targetPivotY=0;
		this.prePivotX=this.targetPivotX;
		this.prePivotY=this.targetPivotY;
		this.txt=new Text();
		this.txt.text=this.tiledMap.width+","+this.tiledMap.height;
		this.txt.fontSize=30;
		this.txt.color="#00FF00";
		this.txt.on("click",this,this.scaleTest);
		this.txt1=new Text();
		this.txt1.text="hello";
		this.txt1.fontSize=30;
		this.txt1.color="#0000FF";
		this.txt1.pos(this.SW()-100,this.SH()-100);
		this.txt1.on("click",this,this.test2);
		Laya.stage.addChild(this.txt);
		Laya.stage.addChild(this.txt1);
		this.txt2=new Text();
		this.txt2.text="random pivot";
		this.txt2.fontSize=30;
		this.txt2.color="#FF0f0f";
		this.txt2.pos(this.SW()-400,this.SH()-60);
		this.txt2.on("click",this,this.test3);
		Laya.stage.addChild(this.txt2);
		this.txt1.text=this.tiledMap.mapSprite().width+",,"+this.tiledMap.mapSprite().height;
		this.txt2.graphics.drawRect(this.MP().x,this.MP().y,this.MP().width,this.MP().height,"#FF0000","#FF00FF",2);
		this.txt2.zOrder=10000;
		if(Browser.onPC){
			this.txt.text="pc";
			}else if(Browser.onAndriod){
			this.txt.text="Android";
	...
	__proto.test=function(){
		console.log("map:"+this.tiledMap.tileWidth);
	}
	...
	Laya.__init([LoaderManager,EventDispatcher,TiledMap,Render,View,Timer,GraphicAnimation,LocalStorage,Browser]);
	/**LayaGameStart**/
	new LayaUISample();
